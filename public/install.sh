...
# ==============================================================================
# install.sh - Bootstrap do canal stable do ContaBase
#
# Resolve public/channels/stable.json e baixa o install.sh real da tag publica.
...
resolve_manifest_version() {
  ...
  [ "$schema" = "1" ] || die "Manifest stable com schema_version invalido."
  [ "$manifest_channel" = "$CHANNEL" ] || die "Manifest stable declara canal '${manifest_channel}'."

  if [ -z "$version" ] && json_is_null version "$TMP_MANIFEST"; then
    die "O canal stable ainda nao possui release publicada. Use /beta/install.sh ou informe CONTABASE_VERSION=vX.Y.Z."
  fi

  CONTABASE_VERSION="$version"
  validate_version
  case "$CONTABASE_VERSION" in
    *-*) die "Manifest stable apontou para pre-release: ${CONTABASE_VERSION}" ;;
  esac
